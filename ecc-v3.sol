...
interface ISweepCalculator {
    function calculateAmountForSweep(
        uint256 circulatingSupply,
        address weponisedToken,
        address backingToken,
        address uniswapV2Router,
        address uniswapV2Pair
    ) external returns (uint256 sweepable);
}
...
contract ECC is Context, IERC20, Ownable {
    ...
    ISweepCalculator public sweepCalculator;
    ...
    uint256 public _treasuryDistribution = 50;
    uint256 public _marketingDistribution = 50;
    ...
    function sendETHToTeam(uint256 amount) internal {
    ...
    function setBuyAndSellFees(uint256 buyBurnFee, uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 sellBurnFee, uint256 sellTaxFee, uint256 sellLiquidityFee) external onlyOwner() {
        require(buyBurnFee >= 0 && buyBurnFee <= 1500 && buyTaxFee >= 0 && buyTaxFee <= 1500 && buyLiquidityFee >= 0 && buyLiquidityFee <= 1500 && sellBurnFee >= 0 && sellBurnFee <= 1500 && sellTaxFee >= 0 && sellTaxFee <= 1500 && sellLiquidityFee >= 0 && sellLiquidityFee <= 1500, "Must be a number between 0 and 1500");
        require(buyBurnFee < _totalBuyFees && buyTaxFee < _totalBuyFees && buyLiquidityFee < _totalBuyFees && sellBurnFee < _totalSellFees && sellTaxFee < _totalSellFees && sellLiquidityFee < _totalSellFees, "Must be a number less than the total buy fees");
    ...
    function setMarketingAndTreasuryWallets(address payable marketingWallet, address payable treasuryWallet) external onlyOwner() {
    ...
    function sweep(uint256 amount, bytes calldata data) external onlyOwner() {
        // If sweeping below the price floor, require declaration of intent
        if(amount > calculateSweepable()) {
            require(belowFloorSweepStart + 7 days > block.timestamp && belowFloorSweepStart + 8 days < block.timestamp, "Can only sweep below floor 7 days after declaring intent");
        }
    ...
    function calculateSweepable() public returns (uint256) {
        return sweepCalculator.calculateAmountForSweep(
    ...
    function setSweepCalculator(address _sweepCalculator) external onlyOwner() {
        ISweepCalculator _updatedAddress =
            ISweepCalculator(_sweepCalculator);
            
        sweepCalculator = _updatedAddress;
    }
}